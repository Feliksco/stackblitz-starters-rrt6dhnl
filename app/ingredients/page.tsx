        ...
        <header className="flex justify-between items-end mb-8">
          ...
          <div className="flex gap-4">
            <input 
              ...
              className="p-4 rounded-2xl border-none ring-1 ring-slate-200 w-64 shadow-sm focus:ring-2 focus:ring-emerald-500 outline-none"
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-100 text-[10px] uppercase font-bold text-slate-400 tracking-widest">
              <tr>
                ...
                <th className="p-6 text-center">Price ($/kg or L)</th>
                <th className="p-6 text-center">Energy (KJ/100g)</th>
                ...
                <tr key={ing.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="p-6">
                    ...
                    {ing.price_per_kg_l === 0 && (
                      <span className="ml-3 bg-red-50 text-red-500 text-[8px] px-2 py-0.5 rounded-full font-black uppercase">Missing Price</span>
                    )}
                  </td>
                  <td className="p-6 text-center font-medium text-slate-600">${ing.price_per_kg_l?.toFixed(2)}</td>
                  <td className="p-6 text-center font-medium text-slate-400">{ing.kj_per_100} <small>KJ</small></td>
          ...
          <div className="bg-white rounded-[2.5rem] p-10 max-w-md w-full shadow-2xl">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input 
                  className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
                    value={editingIng.price_per_kg_l}
                    onChange={e => setEditingIng({...editingIng, price_per_kg_l: e.target.value})}
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="number"
                    className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
                    value={editingIng.kj_per_100}
                    onChange={e => setEditingIng({...editingIng, kj_per_100: e.target.value})}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-4 bg-slate-50 rounded-xl border-none ring-1 ring-slate-200 focus:ring-2 focus:ring-emerald-500 outline-none font-bold"
                  value={editingIng.weight_per_unit}
                  onChange={e => setEditingIng({...editingIng, weight_per_unit: e.target.value})}
                />
                <p className="text-[10px] text-slate-400 mt-2 italic">Example: 1 Egg = 0.05kg</p>
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-10">
              <button 
                ...
                className="flex-1 p-4 rounded-xl font-bold text-slate-400 hover:bg-slate-50 transition"
              >
                Cancel
              </button>
              <button 
                onClick={handleSave}
                className="flex-1 p-4 bg-emerald-600 text-white rounded-xl font-black shadow-lg hover:bg-emerald-700 transition transform active:scale-95"
              >
                Save Changes
              </button>
            </div>
